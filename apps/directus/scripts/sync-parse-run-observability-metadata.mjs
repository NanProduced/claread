...
const DASHBOARDS = [
  {
    ...
    icon: "monitoring",
    color: "#245CB8",
    note: "解析链路观测首页：健康、队列、失败、质量降级、usage 和最近失败任务。",
  },
];
...
const DASHBOARD_PANELS = [
  ...
  {
    id: "4ad98e26-314a-4f6f-a7b1-2d5f85b8e107",
    dashboard: PARSE_RUN_DASHBOARD_ID,
    name: "解析健康总览",
    icon: "monitor_heart",
    type: "claread-parse-run-health-overview",
    position_x: 1,
    position_y: 1,
    normal_position_x: 1,
    normal_position_y: 43,
    width: 24,
    height: 14,
    color: "#245CB8",
    show_header: true,
  ...
  {
    id: "4ad98e26-314a-4f6f-a7b1-2d5f85b8e108",
    dashboard: PARSE_RUN_DASHBOARD_ID,
    name: "失败类型分布",
    icon: "report",
    type: "claread-parse-run-failure-types",
    position_x: 49,
    position_y: 1,
    normal_position_x: 49,
    normal_position_y: 43,
    width: 24,
    height: 14,
    color: "#BE123C",
    show_header: true,
    note: "近 7 天 failed 任务按 failure_code 聚合 Top 5。",
    options: {
      endpointUrl: "/parse-run-observability/summary?days=7",
      targetUrl: "/admin/content/analysis_tasks/+?bookmark=Analysis%20Tasks%20%2F%20Failed",
    },
  },
  {
    id: "4ad98e26-314a-4f6f-a7b1-2d5f85b8e109",
    dashboard: PARSE_RUN_DASHBOARD_ID,
    name: "队列与卡死任务",
    icon: "pending_actions",
    type: "claread-parse-run-queue-health",
    position_x: 25,
    position_y: 1,
    ...
    color: "#9A5B00",
    show_header: true,
    note: "queued / running / finalizing 和超过 5 分钟 stale 的任务。",
    options: {
      endpointUrl: "/parse-run-observability/summary?days=7",
      targetUrl: "/admin/content/analysis_tasks",
    },
  ...
  {
    id: "4ad98e26-314a-4f6f-a7b1-2d5f85b8e110",
    dashboard: PARSE_RUN_DASHBOARD_ID,
    name: "质量降级分布",
    icon: "rule",
    type: "claread-parse-run-quality-degradation",
    position_x: 1,
    position_y: 31,
    normal_position_x: 1,
  ...
  {
    id: "4ad98e26-314a-4f6f-a7b1-2d5f85b8e111",
    dashboard: PARSE_RUN_DASHBOARD_ID,
    name: "模型成本趋势",
    icon: "query_stats",
...
function buildPanelsInsertSql() {
  function placement(item, field) {
    const normalField = `normal_${field}`;
    return RESET_PARSE_RUN_DASHBOARD ? item[field] : (item[normalField] ?? item[field]);
  }
  ...
  return `
    ...
    VALUES
      ${DASHBOARD_PANELS.map(
        (item) =>
          `(${sqlLiteral(item.id)}, ${sqlLiteral(item.dashboard)}, ${sqlLiteral(item.name)}, ${sqlLiteral(item.icon)}, ${sqlLiteral(item.color)}, ${item.show_header ? "TRUE" : "FALSE"}, ${sqlLiteral(item.note)}, ${sqlLiteral(item.type)}, ${placement(item, "position_x")}, ${placement(item, "position_y")}, ${placement(item, "width")}, ${placement(item, "height")}, ${sqlLiteral(JSON.stringify(item.options))}::json)`,
      ).join(",\n      ")}
    ON CONFLICT (id) DO UPDATE SET
