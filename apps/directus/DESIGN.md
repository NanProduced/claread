...
它的默认承载是 Directus Data Studio，因此整体视觉应优先靠近 Directus 原生工作台：干净、克制、可维护、利于高频操作。与此同时，规划中的 Console 诊断模块（如 runtime span / trace 诊断、workflow eval、RAG / few-shot editor）需要更强的分层、异常高亮、证据联动和下钻能力，接近 LangSmith / Sentry 的问题详情页。当前这些深诊断模块尚未实现。
