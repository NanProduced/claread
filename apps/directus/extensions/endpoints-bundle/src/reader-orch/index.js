...
// Independent Reader observability owner.
// Full paths (Directus endpoint name prefix + route):
...
// Read-only contract preserved from former parse-run-observability/reader-orch.js.
export default (router, { database }) => {
  router.get("/trace/:trace_id", async (req, res, next) => {
  ...
  router.get("/run/:run_id", async (req, res, next) => {
    if (!buildAuthGuard(req, res)) return;
  ...
  router.get("/record/:record_id/summary", async (req, res, next) => {
    if (!buildAuthGuard(req, res)) return;
