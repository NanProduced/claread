...
本文规划 Claread Web 端从可运行基线走向完整产品形态的实施路径。开发顺序可以先从功能页和 Reader 启动，但正式产品与设计总纲覆盖的是整站 Web：功能页、Reader、未来产品页、分享页、导出页和内容型页面都必须说同一门语言。Web 首期目标不是重做小程序，而是在共享后端和 canonical result 的前提下，建立更完整的 Web 阅读体验。
...
- `apps/web/PRODUCT.md` / `apps/web/DESIGN.md`：Web 整站产品与设计总纲。
- `apps/web/docs/design/`：Web UI 方向探索。

Web 首期实施遵循：

- 开发顺序可以先从功能页面和 Reader 启动，但正式总纲必须先覆盖整站。
...
- `/read` 是当前 Web app home；`/` 现阶段保留极简入口，但这不代表产品页不属于正式设计系统。
- 未登录不开放模型试用；后续可展示少量精选解析示例，不使用 mock fixture 或真实提交回退。
- `/read` 展示最近记录，但只作为克制索引，不做 feed。
- Reader 只有 `精读` 与 `沉浸` 两模式。默认使用精读模式；模式切换表达阅读意图，不是显示 preset。
...
- 阅读设置只保留 `字号 / 字体 / 主题` 三项阅读校准；不再暴露预设、译文显隐、行距、版心、纸面主题或标注层开关。
- `纸质 Paper` 是 Claread Web 的母主题，也是全站默认主题；`浅色 Light` 更功能化，`深色 Dark` 更夜读。
- 用户批注当前实现支持句子级、单句内 `text_range` 和跨句/跨段 `multi_text`；但产品层只把句子级和单句内 `text_range` 视为稳定方向，`multi_text` 仍作为受限高级能力继续观察，不再单独引出“学习资产中心”心智。PDF/外部网页 anchor 和富文本笔记继续后置。
...
- 精读与沉浸必须作为两套明确 projection 推进；沉浸模式是 paragraph-first 的杂志式排版，不是精读模式减配。
- 阅读设置只允许调整主题、字体和字号，不再承担模式控制。
- Reader 偏好持久化。
...
当前目录结构已落地，详见 `apps/web/src/`。技术栈细节见 `tech-stack-options.md`。
