...
- `apps/web/src/lib/mock-data.ts` 只保留 history fallback 和 reader demo 数据；vocabulary 和 quota mock 已从主路径移除。
...
- `/vocabulary` 已通过 Web BFF 接入 FastAPI `GET /vocabulary`，匿名、mock 登录和上游不可用时显示明确空态，不再展示 mock 生词。
- `/review` 已通过 Web BFF 接入 FastAPI `GET /vocabulary/review/due` 和 `POST /vocabulary/{id}/review`，支持真实待复习队列和复习结果提交。
- 尚未接入 TanStack Query、生词写入、收藏、反馈和用户批注。手机号登录链路已具备开发期 mock、Web BFF cookie 投影和 FastAPI `aliyun_dypnsapi` provider，后续重点是补齐小程序 baseline 的剩余 API 和正式账号绑定 UI。
...
| 生词本 | `/vocabulary` | vocabulary APIs | 已接真实列表；下一步支持写入 |
| 生词复习 | `/review` | review APIs | 已接真实 due queue 和提交 |
