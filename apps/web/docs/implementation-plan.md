...
- Reader 是 Web 品牌第一现场。
- Reader 工作台：中心原文画布 + 左侧常驻词典/动态操作区 + 右侧 AI 工作区。
...
- Reader 已接入收藏、生词写入、句子级高亮/笔记；Library 已接入真实记录删除；Settings 已接入应用反馈。手机号登录链路已具备开发期验证码、Web BFF cookie 投影和 FastAPI `aliyun_dypnsapi` provider，后续重点是补齐 Daily Reader、资料筛选、正式账号绑定 UI 和 Reader UI/UX 评审。
- Reader UI/UX 第一轮已废弃右侧轻旁注方向，转为设计图 `web-v1-reader-permanent-dictionary-direction.png` / `web-v1-reader-settings-dynamic-panel-direction.png` / `web-v1-reader-ai-workspace-direction.png` 对齐的工作台形态：左侧常驻词典 + 左下动态操作区 + 中心原文画布 + 右侧 AI 工作区。
...
- Reader 桌面端使用左侧工具区：左上常驻词典面板实时显示当前点击词/短语详细释义，原文附近只显示轻释义预览；左下动态操作区在句子选择时显示高亮、写笔记、收藏、问 Claread，在点击 `Aa` 时显示阅读设置。`grammar_note` 语法说明和 `sentence_analysis` 句子拆解应在相关句子下方展开，不使用 Grammar X-Ray 命名。
...
- 点击/hover/选中触发轻浮层。
- 原文附近显示轻释义预览；词汇详细释义进入左上常驻词典面板；句子操作和阅读设置进入左下动态面板；语法说明、句子拆解和用户笔记回到中心原文画布。
