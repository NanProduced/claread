...
- Reader 已接入收藏、生词写入、句子级高亮/笔记，以及单句内 `text_range` 高亮/笔记/收藏；Library 已接入真实记录删除；Settings 已接入应用反馈。手机号登录链路已具备开发期验证码、Web BFF cookie 投影和 FastAPI `aliyun_dypnsapi` provider，后续重点是补齐 Daily Reader、资料筛选、正式账号绑定 UI 和 Reader UI/UX 评审。
...
- 用户批注 v1 支持句子级和单句内 `text_range`；跨句选区、PDF/外部网页 anchor 和富文本笔记后置。
...
| 收藏 | Reader / history | favorites APIs | Reader 记录收藏、句子收藏和局部 `text_range` 收藏已接入 |
| 生词本 | `/vocabulary` | vocabulary APIs | 已接真实列表；Reader 可写入 |
| 生词复习 | `/review` | review APIs | 已接真实 due queue 和提交；不放一级导航，从 `/vocabulary` 进入 |
| 批注 | Reader | user-annotations APIs | 已接句子级和单句内 `text_range` 高亮/笔记；跨句选区后置 |
...
- Reader 偏好持久化。
- SelectionToolbar 稳定化：Playwright smoke 覆盖出现位置、滚动跟随、创建/取消高亮、笔记和局部收藏。
