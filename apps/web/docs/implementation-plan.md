...
- Reader 已接入收藏、生词写入、句子级高亮/笔记，以及单句内 `text_range`、跨句/跨段 `multi_text` 高亮/笔记/收藏；`/library/assets` 已按文章聚合句子、局部文本和多段文本学习资产；Library 已接入真实记录删除；Settings 已接入应用反馈。手机号登录链路已具备开发期验证码、Web BFF cookie 投影和 FastAPI `aliyun_dypnsapi` provider，后续重点是补齐 Daily Reader、资料筛选、正式账号绑定 UI 和 Reader UI/UX 评审。
