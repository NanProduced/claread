...
- Reader 桌面端使用画布边缘工具层：画布左侧词典详情卡片实时显示当前点击词/短语详细释义，原文附近只显示可关闭的轻释义预览；句子选择时通过短时浮层显示高亮、写笔记、收藏、问 Claread，点击 `Aa` 时显示阅读设置。`grammar_note` 语法说明和 `sentence_analysis` 句子拆解应在相关句子下方展开，不使用 Grammar X-Ray 命名。
