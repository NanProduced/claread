...
export function HeroCopy({ ctaHref = "/", ctaLabel = "开始透读" }: { ctaHref?: string; ctaLabel?: string }) {
  return (
    <div className="relative z-20 flex w-fit flex-col items-start text-left">
      <style>{`
      ...
      <h1 data-hero-type className="relative w-full font-headline text-ink">
        <span className="relative block w-fit max-w-full whitespace-nowrap text-[clamp(2.8rem,11vw,4rem)] font-semibold leading-[0.92] tracking-[0.01em] sm:text-[clamp(4.2rem,7.5vw,6.5rem)]">
          透
        ...
        <span className="relative mt-4 block w-full text-[clamp(2.6rem,10vw,3.8rem)] font-normal leading-[0.92] tracking-[-0.025em] sm:mt-5 sm:text-[clamp(4rem,7vw,6.5rem)] sm:leading-[0.9]">
          ...
          <HeroProofPin
            label="3"
            color="#1f5eff"
            className="left-[52.2%] top-[1.15em]"
      ...
      <div className="mt-8 flex w-full">
        <p className="max-w-[34rem] text-[clamp(1.1rem,1.5vw,1.35rem)] font-medium leading-8 text-ink-soft sm:leading-9">
          一句一句看清语法、结构和意思。
        </p>
      </div>

      <div className="mt-10 flex w-full">
        <Link href={ctaHref} className="focus:outline-none focus-visible:ring-2 focus-visible:ring-ink/20 focus-visible:ring-offset-2 rounded-full">
          <ShimmerButton
            background="var(--ink)"
            shimmerColor="rgba(255,255,255,0.7)"
            shimmerSize="0.08em"
            shimmerDuration="3s"
            className="group flex items-center gap-2 px-7 py-3 text-[0.95rem] font-semibold transition-all duration-300 hover:scale-105 hover:shadow-[0_0_20px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_0_20px_rgba(255,255,255,0.1)] active:scale-95"
          >
            <span style={{ color: "#ffffff" }}>{ctaLabel}</span>
            <ArrowRightIcon className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" style={{ color: "#ffffff" }} />
          </ShimmerButton>
        </Link>
      </div>
    </div>
