...
export function AnnotationGutter({ annotations }: AnnotationGutterProps) {
  ...
  return (
    <div
      className="absolute -left-5 top-2.5 flex flex-col gap-1"
      aria-hidden="true"
    >
      {hasNote ? (
        <div className="relative text-vocab-amber drop-shadow-sm transition-transform hover:-translate-y-0.5">
          <Bookmark className="h-5 w-5 fill-current" />
        </div>
      ) : hasHighlight ? (
        <div className="relative text-structure-green drop-shadow-sm opacity-80">
