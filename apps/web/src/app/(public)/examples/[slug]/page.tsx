...
import { appReadRoute, dailyRoute, examplesRoute, loginRoute } from "@/lib/routes";
  ...
  return (
    <main className="min-h-screen bg-[oklch(97%_0.012_84)] px-5 py-6 text-ink sm:px-8 lg:px-12">
      <div className="mx-auto max-w-5xl">
        <PublicSiteHeader currentHref={examplesRoute} priority />
        <header className="mt-8 flex items-center justify-between gap-6">
          <Link href={dailyRoute} className="focus-ring inline-flex items-center gap-2 rounded-pill text-sm font-semibold text-muted hover:text-ink">
        ...
        <article className="reading-paper mt-8 rounded-[2rem] border border-hairline px-6 py-8 shadow-[0_28px_80px_rgba(35,28,18,0.12)] sm:px-12 sm:py-12">
          ...
          <section className="mt-10 grid gap-4 border-t border-hairline pt-6 md:grid-cols-2">
            ...
            <div>
              ...
              <Link
                href={session.state === "signed_out" ? loginRoute(appReadRoute) : cta.href}
              ...
              >
                ...
                {session.state === "limited_debug" ? "打开调试工作区" : session.state === "signed_in" ? "打开 Claread" : "解读我的文章"}
              </Link>
            </div>
          </section>
        </article>
