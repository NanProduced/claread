...
export function FeedbackForm() {
  ...
  return (
    <form
      className="rounded-note border border-hairline bg-surface p-5 shadow-surface-quiet"
      onSubmit={handleSubmit}
    >
      <div className="flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          <label className="font-semibold text-ink text-[0.9375rem]" htmlFor={contentId}>
            应用反馈
          </label>
            ...
            <button
              className={`rounded-md border px-3 py-2 text-sm font-semibold transition ${
                feedbackType === option.value
                  ...
                  : "border-hairline bg-web-canvas text-muted hover:text-ink"
              }`}
              key={option.value}
        ...
        <textarea
          className="min-h-28 w-full resize-y rounded-md border border-hairline bg-web-canvas px-3 py-2 text-sm leading-6 text-ink outline-none focus:border-lens-blue"
          id={contentId}
          maxLength={2000}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            className="rounded-pill bg-ink px-5 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:bg-muted"
            disabled={!canSubmit}
            type="submit"
          >
