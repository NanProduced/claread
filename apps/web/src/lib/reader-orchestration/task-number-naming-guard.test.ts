 ...
 *    scanned for UPPERCASE task-history code shapes (round/phase/stage/
 *    batch/task letters followed by a digit, plus `ASK-` epic prefixes).
...
// Uppercase task-history code shapes only. Lowercase tokens (fixture ids
// such as quoted sentence/block ids, urls, retired path names, regular
// variables) are not task codes and must not need allowlisting.
const TASK_CODE_LINE_RE = /\b[BRPST][0-9]/;
...
const CODE_IDENTITY_ALLOWLIST_CEILING = 8;
...
describe("Naming governance guard: task numbers stay out of Web test names and sources", () => {
  ...
  it("file-name matcher rejects task-numbered names and accepts business names", () => {
    const mustMatch = [
      "r1-example.test.ts",
      "feature.p2c.test.ts",
      "feature_t4.2a.test.ts",
    ];
    const mustPass = ["reader-ask-v2", "article-g5", "l1-heading"];
  ...
  it("code identity allowlist equals its ratchet ceiling and has no stale entries", () => {
    const allText = scannedLines.map(({ text }) => text).join("\n");
    const stale = CODE_IDENTITY_ALLOWLIST.filter(
      (identity) => !allText.includes(identity),
    );
