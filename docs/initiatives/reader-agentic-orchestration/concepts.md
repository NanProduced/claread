...
| Anchor Segment | 锚点片段；sentence-like segment | Reading Unit 内稳定的 span 锚点单位，通常是句子；在无可靠句子边界时可为 clause 或 fallback window。承载用户选区、笔记、高亮和 span-bound layers 的局部 UTF-16 offsets。 | 由 Reading Base Builder 生成；同一 base 内不可变。 | 目标：`anchor_segments`，字段含 `anchor_segment_id`/兼容 `sentence_id`、`segment_type`、base absolute offsets、`text_hash`。 | 不等于 Reading Unit；`segment_type = clause/fallback_window` 时不应称为真实句子。 |
| Text Coordinate Scope | 文本坐标范围；坐标作用域 | 文本坐标的参照范围：unit 使用 Stable Base absolute UTF-16 offsets；span anchor 使用 Anchor Segment local UTF-16 offsets。 | 由 contracts 强制；Publisher 和 User Editing Surface 必须校验。 | 现有：`services/api/app/contracts/annotation.py`、`packages/contracts`；算法 `fnv1a32-utf16`。 | 不允许把全文 offset 写入 segment-local 字段。 |
...
| Grammar Note | 语法旁注；grammar_note | Span-bound 语法点说明，必须锚定到一个或多个 Anchor Segment local ranges。 | Grammar Bundle Worker 或专用 worker 生成；Layer Publisher 独立校验。 | 目标：`enhancement_layers.layer_type = grammar_note`。 | 不等于 sentence_analysis；不是整句结构拆解。 |
