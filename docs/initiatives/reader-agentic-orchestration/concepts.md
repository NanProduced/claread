...
` 仅作 soft hint，不作硬约束；planner 只输出 anchor 范围，hydrate/校验由后端拥有。24 篇盲评（translation-grouping-v1）显示语义分组对 deterministic planner 有显著收益（overall 19/24，结构复杂 8/8），两阶段 LLM planner 为后续候选。 | 目标：translation group metadata；`group_id`、`anchor_segment_ids`。 | 不等于 one-unit-one-group；不等于 one-sentence-one-group；不等于 Analysis Window。 |
