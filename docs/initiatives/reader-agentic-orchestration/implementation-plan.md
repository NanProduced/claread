...
6. D5-W3 Web cutover planning 已确认当前双轨矩阵：`/app/read`、active task、Library、Vocabulary source links、command palette 与 `services/bff/analysis.ts` 仍指向旧 `/app/reader/{recordId}` / `/analysis-tasks`；`/app/reader-plate?record_id=` 独立消费 `ReaderPlateSnapshot`，只作为验证入口。W3-B 已先落地 route helper split：legacy 入口显式走 `legacyAppReaderRoute(...)`，reader-plate validation path 显式走 `appReaderPlateRoute(...)`，但没有切产品路由，也没有做旧 `render_scene_json` 兼容映射。详细矩阵、touched files 与 done criteria 见 `modules/cutover-and-old-workflow.md`。
