...
6. D5-W3 Web cutover planning 已确认当前双轨矩阵：`/app/read`、active task、Library、Vocabulary source links、command palette 与 `services/bff/analysis.ts` 仍指向旧 `/app/reader/{recordId}` / `/analysis-tasks`；`/app/reader-plate?record_id=` 独立消费 `ReaderPlateSnapshot`，只作为验证入口。W3-B 已落地 route helper split：legacy 入口显式走 `legacyAppReaderRoute(...)`，reader-plate validation path 显式走 `appReaderPlateRoute(...)`。W3-C1 进一步新增了 `/app/reader-record/{recordId}` product route shell 与 `appReadingRecordRoute(...)` helper，但当前只复用 reader-plate snapshot BFF 做 read-only 直达加载，未切 `/app/read`、Library、Vocabulary、active task 或 command palette 流量，也没有做旧 `render_scene_json` 兼容映射。后续 UI/UX 方向是保留现有 ReaderWorkbench 框架，只替换中心文章正文与标注显示区域为新 `ReaderPlateSnapshot` / Plate 数据面，并补齐锚点交互桥；不要把 reader-plate 单列验证页当最终产品 UI。详细矩阵、touched files 与 done criteria 见 `modules/cutover-and-old-workflow.md`。
