...
- UI-D6B 对默认 Plate surface 做端到端真实流程验收与 UI/UX 收敛评估：对比旧 `ReaderWorkbench.tsx` 基线，修复 `ReaderRecordPlateSurface.tsx` 中英文混排状态文案错误（操作按钮、写入状态、复制状态、section 标签、状态文本、词典面板、按钮标签、aria-label、disabledReason 全部收敛为中文），新增 smoke guard 防止英文 UI 文案回归；未接旧 `/scene`、`render_scene_json`、`analysis-tasks`；未修复但记录的后续 UI/UX 项见 `reader-record-plate-surface-ui.md` UI-D6B 节。
