...
**证据分层（不得混称）**：下表的每一行属于且仅属于一种证据类型；只有“生产集成链”行能支撑“真实 DB→Runtime→SSE→下一轮 prompt 消费”的结论，其余行各自只证明其层。
...
- **scripted UI**：`reader-record-ask-process-target-r0.spec.ts` / `ask-chain-of-thought.spec.ts`，harness 脚本喂 SSE，证真实 `TurnProcessDisclosure` 组件对 compaction 事件的渲染（非生产集成）。
- **production integrated chain**：`test_context_compaction_integrated_chain.py`，真实 PostgreSQL + 真实 `service→runtime→manager→production_stream` + 确定性 fake compactor/answer model（零 provider），证压缩触发→真实 compaction SSE→`reader_ask_thread_memory` CAS 写入→下一轮 model-visible prompt 含 compacted memory + recent history 且 turn 覆盖不重叠→compactor 失败 Host fallback 主答仍完成→隐私。
...
| 生产集成链（真实 PG canonical history→真实 service/runtime/manager/production_stream→确定性 fake compactor→memory 写入→真实 compaction SSE→下一轮 prompt 消费 memory+recent 且不重叠；+fallback；+隐私） | production integrated chain | `CLAREAD_RUN_THREAD_MEMORY_DB_TESTS=1 pytest .../test_context_compaction_integrated_chain.py` | 2 passed |
...
| UI compaction 生命周期（同 disclosure、冷加载不恢复过程卡） | scripted UI | `playwright ... reader-record-ask-process-target-r0.spec.ts` | 22 passed |

> 浏览器对同一条真实 compaction SSE 的消费（Next BFF→浏览器）：真实组件渲染由 scripted-UI 行覆盖；真实页面 UAT 亦观察到压缩感知后继续流式回答。**未**另建“零 provider 的浏览器 compaction”harness：真实 `production_stream` 需会话用户拥有的 Reading Record，而不经 LLM intake 流程就无法合成该记录（或需 owner-login 脚手架，且与并行 Markdown runner 的 next-env 写入竞争）；故该浏览器跳由 production-integrated-chain 行（服务端发射+持久化+消费）+ scripted-UI 行（组件渲染）联合证明。
