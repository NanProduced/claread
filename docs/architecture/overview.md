...
Claread 是多端英文阅读辅助产品。当前迁移基线包含一个 FastAPI 后端、一个微信小程序客户端、本地 PostgreSQL/Redis 开发环境和词典数据资产。
...
其中 `apps/miniprogram/`、`services/api/`、`infra/`、`docs/` 是 clean import 第一阶段重点。`apps/web/`、`apps/directus/`、`services/worker/`、`packages/`、`evals/` 是后续扩展位置。
...
| `apps/miniprogram` | 微信小程序客户端，当前冻结基线 |
