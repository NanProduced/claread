...
- [ ] Serena memory 与当前代码、测试和正式文档无明显滞后或漂移。
      验证：按当前文档工作涉及的子项目激活对应 Serena 项目，查看相关 memory；如果 memory 描述已过期，以代码、测试、正式文档或对应 `AGENTS.md` 为准，更新或删除过期 memory。不要为了记录临时进度新增 memory。
...
4. 对齐当前范围内的 Serena memory：只修正或删除已确认滞后/漂移的 memory，不把临时进度写成长期 memory。
5. 记录发现的问题，按 D0-D3 分级。
6. 默认只报告，不修改；如用户授权，才对低风险项执行修复。
7. 对 D0 给出明确证据、影响范围和建议修改位置。
8. 对 D1-D2 给出清理建议，标明是否需要用户确认。
9. 只有完整执行目标检查且无未处理 D0/D1 时，才更新本文档的"最后验证"日期。
10. 向用户汇报：问题数、已修数、待确认数、跳过项和未运行验证命令。
