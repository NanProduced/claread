...
- `apps/directus/`：内部数据、运营、评测和样本管理工具，后续评估建设。
...
微信小程序和 Web baseline 当前共享通用后端、本地 PostgreSQL/Redis 和词典数据。Directus、LLM-as-a-Judge 和 Few-shot RAG 是后续建设方向。当前状态细节见 `docs/product/current-state.md`。
