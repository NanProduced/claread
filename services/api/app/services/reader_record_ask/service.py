...
async def _safe_close_model(model: Any) -> None:
    """Close an owned model (enter/exit pair → HTTP client shutdown).

    Delegates to the canonical sanitized helper in ``execution_config``;
    failures never mask the caller's original exception.
    """
    await _close_owned_model(model)
...
async def _resolve_agentic_execution(
    ...
    *,
    ...
    try:
        ...
        # Post-build resolver failure: the model was already constructed.
        # Close it here (the only async owner at this point) and keep the
        # typed unavailable/503 contract — no raw text leakage.
        failed_model = getattr(exc, "_failed_model", None)
        if failed_model is not None:
            await _safe_close_model(failed_model)
        logger.warning(
            "agentic_execution_unavailable code=%s model_key=%s reason=%s",
            "model_unconfigured",
            exc.option_key,
            exc.reason,
        )
    ...
    *,
    ...
    try:
        ...
        retry_snapshot = build_retry_snapshot(
            ...
            focus_anchors=[anchor.model_dump(mode="json") for anchor in validated_focus_anchors]
            or None,
        )
        ...
        await _safe_close_model(execution.model)
        raise
...
async def _stream_agentic_v2(
    *,
    ...
    if prepared is not None:
        ...
        model_option_key = execution.option_key if execution is not None else request.model
    ...
    async with AsyncExitStack() as stack:
        if hasattr(model, "__aenter__") and hasattr(model, "__aexit__"):
            # Ownership handoff: this generator becomes the sole owner
            # from the first anext on; the response-level early-close
            # guard then refuses to close it again (explicit state).
            claim = getattr(execution, "claim_model", None)
            if callable(claim):
                claim()
        ...
        web_search_backend = resolved_backend if web_search_capability is not None else None
        ...
        inner_stream = stream_agentic_thread_message(
            user_id=user_id,
            reading_record_id=reading_record_id,
            thread_id=thread_id,
...
async def _revalidate_snapshot_focus_anchors(
    *,
    ...
    for index, anchor in enumerate(parsed):
        anchor_record_id = _parse_uuid(anchor.record_id, field=f"focus_anchors[{index}].record_id")
        if anchor_record_id != reading_record_id:
            raise HTTPException(
                status_code=409,
                detail={
...
async def _load_replayed_web_search_mode(
    *,
    ...
    try:
        ...
        logger.warning(
            "retry_replay_web_search_mode_failed code=db_read_failed thread_id=%s message_id=%s",
            thread_id,
            message_id,
        )
        raise HTTPException(
...
async def retry_reading_record_ask_message(
    *,
    ...
    retry_backend = execution.web_search_backend if retry_capability is not None else None
    async with AsyncExitStack() as stack:
        if hasattr(execution.model, "__aenter__") and hasattr(execution.model, "__aexit__"):
            # Ownership handoff (mirrors the send path): the generator
            # claims before entering so the response-level early-close
            # guard never double-closes.
            claim = getattr(execution, "claim_model", None)
            if callable(claim):
                claim()
            await stack.enter_async_context(execution.model)
        inner_retry = retry_agentic_thread_message(
            user_id=user_id,
            reading_record_id=parsed_record_id,
            thread_id=thread_id,
            message_id=message_id,
            ...
            lifecycle=lifecycle,
            # Usage-accounting identity from the same execution family as the
            # original send — the new turn_run's usage event carries the same
            # model / billing identity.
            execution_snapshot=execution.snapshot,
        )
        try:
            async for chunk in inner_retry:
                yield chunk
        finally:
            await inner_retry.aclose()
