...
from app.services.reader_record_ask import config as cfg
...
def _resolve_option(
    ...
    *,
    ...
    return ResolvedReaderAskModelOption(
        ...
        main_model_name=_resolved_model_name(
            settings, MODEL_ROUTE_READER_ASK, option.selection
        ),
        replan_model_name=_resolved_model_name(
            settings, MODEL_ROUTE_READER_ASK_REPLAN, option.selection
        ),
...
def _enabled_options(
    catalog: ReaderAskModelCatalogConfig,
) -> list[tuple[str, ReaderAskModelOptionConfig]]:
    return [
        (key, option)
...
def list_reader_ask_model_options(
    settings: Settings,
) -> tuple[list[ResolvedReaderAskModelOption], str]:
    catalog = build_reader_ask_model_catalog(settings)
    default_option = resolve_default_reader_ask_model_option(settings)
    enabled_options = _enabled_options(catalog)
    if not enabled_options:
