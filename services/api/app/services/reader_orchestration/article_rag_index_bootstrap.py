...
``stable_document_id`` with the same ``plan_content_sha256`` and
``chunk_count``, the service returns an idempotent no-op result with
``idempotent_noop=True``.  If the plan_content_sha256 differs, the
...
def compute_article_rag_index_build_input_hash(
    *,
    ...
    The digest covers ``stable_document_id``, ``base_id``, and
    ``plan_content_sha256``.  No version / fingerprint fields
    participate — the Article RAG index is a single path.
    """
    ...
    async def bootstrap_article_rag_index_in_transaction(
        ...
        *,
        ...
        if existing is not None:
            ...
            if (
                ...
                if existing_job_id is None or existing_reader_run_id is None:
                    raise ArticleRagIndexBootstrapError(
                        ...
                        f"has a null job_id or reader_run_id "
            ...
            raise ArticleRagIndexBootstrapError(
                f"Existing active index run {existing['id']} for "
                f"stable_document_id={plan.stable_document_id} has a "
                f"different plan_content_sha256 "
