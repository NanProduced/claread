...
# BASELINE_SQL, so the full bootstrap schema is just BASELINE_SQL.
...
# ===================================================================
# Test 14: index_version is honoured in operation_fingerprint
# ===================================================================


async def test_custom_index_version_reflected_in_fingerprint(
    index_env: asyncpg.Pool,
) -> None:
    """A custom index_version produces a distinct operation_fingerprint
    and idempotency_key, and a distinct row in the index state table."""
    await _seed_paragraph_environment(index_env)
    service = _build_service(index_env)

    custom_version = "article_rag_index_v2_test"
    result = await service.bootstrap_article_rag_index(
        reading_record_id=_RECORD_ID,
        user_id=_USER_ID,
        index_version=custom_version,
    )
    assert result.index_version == custom_version

    async with index_env.acquire() as conn:
        job = await _fetch_job(conn, job_id=result.job_id)
        assert job["operation_fingerprint"] == (
            f"article_rag_index_build_v1:{custom_version}"
        )
        assert job["idempotency_key"] == (
            f"article_rag_index_build_v1:{_STABLE_DOC_ID}:{custom_version}"
        )
        index_run = await _fetch_index_run(conn, index_run_id=result.index_run_id)
        assert index_run["index_version"] == custom_version
