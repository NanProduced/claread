...
from .store import AsyncpgJudgeRunRequestStore, JudgeRunRequest, JudgeRunRequestStore


@dataclass
class JudgeWorker:
    store: JudgeRunRequestStore
    evals_root: Path
    worker_id: str
    ...
    async def recover_stale_requests(self) -> list[JudgeRunRequest]:
    ...
    async def run_once(self, *, dry_run_claim: bool = False) -> bool:
        if dry_run_claim:
            ...
            if request is None:
                print("No queued judge request is available.")
                return False
    ...
    async def _execute_claimed_request(self, request: JudgeRunRequest) -> None:
        ...
        try:
            max_concurrency = int(request.config_json.get("max_concurrency") or 1)
            if max_concurrency != 1:
                raise ValueError("Judge Worker v1 only supports max_concurrency=1.")

            _, artifact_dir = await run_judge(
                JudgeRunConfig(
                    judge_run_id=request.judge_run_id,
                    run_id=request.run_id,
                    rubric_id=request.rubric_id,
                    rubric_version=request.rubric_version,
                    judge_adapter_kind=request.judge_adapter_kind,
...
def _request_preview(request: JudgeRunRequest) -> str:
    payload = {
        "id": request.id,
        "judge_run_id": request.judge_run_id,
        "run_id": request.run_id,
...
def _artifact_path(artifact_dir: Path) -> str:
    try:
        judge_run_id = artifact_dir.name
        run_id = artifact_dir.parent.parent.name
        return (Path("evals") / "runs" / run_id / "judge" / judge_run_id).as_posix()
    except Exception:
        return artifact_dir.as_posix()


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run queued Claread judge requests.")
    parser.add_argument("--database-url", default=os.environ.get("DATABASE_URL"))
    parser.add_argument("--evals-root", default=Path.cwd())
    parser.add_argument("--worker-id", default=f"eval-judge-worker-{uuid4()}")
...
async def async_main(argv: Sequence[str] | None = None) -> int:
    ...
    store = await AsyncpgJudgeRunRequestStore.connect(args.database_url)
    worker = JudgeWorker(
        store=store,
