...
# P-5A: normalization is single-sourced in the shared stdlib-only package
# under services/api (harness-style sys.path bootstrap; workflow.py's
# stemming `_normalize_highlight_key` intentionally stays separate).
_SERVICES_API_DIR = Path(__file__).resolve().parents[3] / "services" / "api"
if str(_SERVICES_API_DIR) not in sys.path:
    sys.path.insert(0, str(_SERVICES_API_DIR))

from app.services.daily_reader.teaching.normalize import (  # noqa: E402
...
def check_gold_expression_coverage(
    ...
    if artifact.get("abort"):
        return {
            "passed": None,
            "detail": {"coverage": None, "note": "aborted run — no artifacts to evaluate"},
        }
    expected = (case.get("gold") or {}).get("expected_expressions", [])
    if not expected:
        return {"passed": True, "detail": {"coverage": 1.0, "note": "no gold expressions"}}
    candidates = [
        normalize_expression(h.get("text", "")) for h in artifact.get("highlights_json") or []
    ]
    candidates += [
