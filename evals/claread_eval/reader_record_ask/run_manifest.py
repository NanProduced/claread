...
Spec: run-manifest audit contract.
...
#: Audit contract version. Distinguishes manifests that
#: enforce the strict V2 identity/budget contract from legacy V1
#: older manifests. V2 requires:
#:
...
#: dict. This closes the "empty dict bypass" — a V2 manifest with
...
class ReaderRecordAskRunManifest:
    """Persistent run-level completion state for one evaluation phase run.
    ...
    - ``dataset_id`` / ``dataset_schema_version`` / ``dataset_content_sha256``:
      ...
      the snapshot-loading entrypoint). Artifacts MUST carry the
    ...
    # Per-case runtime fixture identity map. The
    ...
    # Empty for backwards compatibility with older manifests.
    runtime_fixture_identities: dict[str, str] = field(default_factory=dict)
    # Self-contained budget audit fields. The harness
    ...
    # env. Defaults preserve backwards compatibility with older
    ...
    # Explicit audit contract version. ``None`` or
    # ``"r4-a4-2r2"`` selects V1 (legacy) compatibility — those
    ...
    def from_json(cls, s: str) -> ReaderRecordAskRunManifest:
        ...
        return cls(
            ...
            # Optional fields — use .get() with defaults so
            # older manifests (which never carried these keys)
            # round-trip cleanly.
...
def read_manifest(manifest_path: Path) -> ReaderRecordAskRunManifest | None:
    ...
    - File exists but bytes are not valid UTF-8 (``UnicodeDecodeError``
      ...
      ``corrupt_manifest``, ``path=str(manifest_path)``). Previously,
      ``read_text(encoding="utf-8")`` propagated the
...
def read_manifest_with_state(
    ...
    (only when valid). The previous code caught
    ...
    - File exists but bytes are not valid UTF-8
      ...
      ``ManifestReadResult(state=CORRUPT)``. Previously,
    ...
    try:
        ...
        # ``UnicodeDecodeError`` / ``UnicodeError`` are handled explicitly:
...
class CoverageAuditResult:
    ...
    - ``manifest_state``: three-state classification
      (``"absent"`` / ``"valid"`` / ``"corrupt"``). A corrupt manifest
    ...
    - ``manifest_run_id_matches``: ``True`` only when the manifest is
      ...
      no valid manifest to compare (absent / corrupt). A
    ...
    # Additional fields. Defaults preserve backward compatibility
...
def _parse_and_validate_manifest_dict(s: str) -> dict[str, Any]:
    ...
    The strict contract has 10 rejection rules. Each rule is checked
    ...
    # Rules 11-16: optional budget/identity fields. Each is
    ...
    # valid for older manifests. When keys
    ...
    # Rule 15: ``retry_policy`` is a dict.
    ...
    # Rule 17: ``audit_contract_version`` — optional
    ...
    # Rule 18: V2 strict contract. When
    ...
    if acv_value == AUDIT_CONTRACT_VERSION_V2:
        ...
        # ``request_cap - planned_logical_runs`` (the retry-headroom
        # contract). Drift indicates a harness bug.
        if rh != rc - plr:
            raise RunManifestError(reason="corrupt_manifest")
...
def _assert_coverage_invariant(data: dict[str, Any]) -> None:
    ...
    Coverage invariants (checked AFTER list-uniqueness is verified in
