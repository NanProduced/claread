...
本仓库是 Claread 多端 monorepo。当前第一阶段从旧微信小程序仓库 clean import 小程序和通用后端，后续继续开发 Web、Directus 内部工具、LLM-as-a-Judge 和 few-shot RAG。
...
- 小程序当前冻结基线只是迁移时的稳定点，不代表 Claread 完整功能上限。
- 不把微信小程序平台限制写成全局产品限制。
...
- clean import 后先跑后端核心测试、小程序构建和 TypeScript 检查，再开始 Web 开发。
